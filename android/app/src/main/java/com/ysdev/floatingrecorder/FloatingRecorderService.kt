...
class FloatingRecorderService : Service() {
    ...
    companion object {
        ...
        fun stop(c: Context) {
            c.stopService(Intent(c, FloatingRecorderService::class.java))
        }
    }
    ...
    override fun onCreate() {
        ...
        instance = this
        createChannel()
        startForeground(NOTIF_ID, buildNotif("Floating Recorder", "Starting..."))
        initMediaProjection()
        showFloatingButton()
    }
    ...
    private fun showFloatingButton() {
        ...
        try {
            ...
            updateNotif("Floating Recorder", "Tap button to start")
        } catch (_: Exception) {}
    }
    ...
    private fun toggleRecording() {
        ...
        if (rec.isRunning()) {
            ...
            rec.onAmplitude = { updateUI(true) }
            rec.onTimeUpdate = { ms ->
                elapsedSec = ms / 1000
                updateNotif("🎙️ Recording", formatTime(elapsedSec))
            }
            rec.onModeChanged = { mode, _ ->
                updateNotif("🎙️ ${mode.display}", formatTime(elapsedSec))
            }
            rec.onSaved = { file ->
